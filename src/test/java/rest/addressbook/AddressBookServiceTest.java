...
public class AddressBookServiceTest {
	...
	public void serviceIsAlive() throws IOException {
		...
		assertEquals(200, response.getStatus());
			List<Person> list1 = response.readEntity(AddressBook.class).getPersonList();
		...
		Response response2 = client.target("http://localhost:8282/contacts")
				.request().get();
		List<Person> list2 = response2.readEntity(AddressBook.class).getPersonList();
	...
	public void createUser() throws IOException {
		...
		// we check POST /contacts is not safe
		...
		// we check POST /contacts is not idempotent
		response = client.target("http://localhost:8282/contacts")
				.request(MediaType.APPLICATION_JSON)
				.post(Entity.entity(juan, MediaType.APPLICATION_JSON));
		juanUpdated = response.readEntity(Person.class);
	...
	public void createUsers() throws IOException {
		...
		//We compare the new Maria with the before Maria.
	...
	public void listUsers() throws IOException {
		...
		response = client.target("http://localhost:8282/contacts")
				.request(MediaType.APPLICATION_JSON)
				.post(Entity.entity(daniel, MediaType.APPLICATION_JSON));
		response = client.target("http://localhost:8282/contacts").request()
				.get();
		List<Person> list2 = response.readEntity(AddressBook.class)
				.getPersonList();
	...
	public void deleteUsers() throws IOException {
		...
		// We did the same operation (delete), and the result was the same, so its idempotent.
		response = client.target("http://localhost:8282/contacts/person/2")
				.request().delete();
		assertEquals(404,response.getStatus());
